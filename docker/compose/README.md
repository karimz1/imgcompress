...
| File | Use case |
|------|----------|
| `advanced.docker-compose.yaml` | Single-host deployment with production best practices (log rotation, healthchecks, resource limits). No reverse proxy included. |
| `proxied.docker-compose.yaml` | Public-facing deployment behind a **Traefik** reverse proxy with automatic HTTPS, HTTP/3, security headers, and network isolation. |
...
- **For public Internet exposure:** Use `proxied.docker-compose.yaml`. It is a battery-included with a fully-configured Traefik reverse proxy that handles TLS termination, HTTPS redirection, and security headers out of the box.
