...
- All non-secret backend deployment config lives in `backend/image_converter/config/app.json`. This is the only source of truth. Read it through the typed accessors in `backend/image_converter/config/settings.py` — never via `os.environ` from feature code, never with fallback defaults in code, and never duplicated across files. Frontend runtime flags are served by the backend route `/config/runtime.json`, which projects the relevant keys from `app.json`; there is no separate frontend config file to keep in sync.
...
- Adding a new tunable: (1) add the key to `app.json` with a real value, (2) add a typed accessor in `settings.py` using `_require_str` / `_require_int` / `_require_bool` / `_require_int_or_auto` with appropriate bounds, (3) append the accessor to `_REQUIRED_GETTERS`, (4) inject it at the composition root, (5) add unit-test cases in `tests/unit/test_settings.py` covering happy path, missing key, wrong type, out-of-range (and bool-as-int rejection for int keys). The PR is not done until those tests exist and pass.
- Distinguish deployment config from process IPC. Variables like `IMGCOMPRESS_STDIO_CAPTURE_INSTALLED`, `IMGCOMPRESS_PARENT_STDOUT_CAPTURE`, and `IMGCOMPRESS_EXTERNAL_STDOUT_TEE` coordinate between a launcher shell or parent Python process and the child granian process; they stay as env vars because that *is* the mechanism. They do not belong in `app.json`. If you find yourself adding a new env var, ask whether it is process-IPC (env is correct) or deployment config (`app.json` is correct) before writing it.
...
- Production image smoke test: `PORT_HOST=8080 ./runLocalDockerBuildTester.sh`, then open `http://localhost:8080`. To toggle feature flags for the smoke run, edit `backend/image_converter/config/app.json` before building, or mount a customized copy over `/container/backend/image_converter/config/app.json`.
