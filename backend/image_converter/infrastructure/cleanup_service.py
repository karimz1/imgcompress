...
class CleanupService:
    ...
    def cleanup_temp_folders(self, force: bool = False):
        current_time = time.time()
        for item in os.listdir(self.temp_dir):
            item_path = os.path.join(self.temp_dir, item)

            if os.path.isdir(item_path) and (item.startswith("source_") or item.startswith("converted_")):
                self._maybe_delete_dir(item_path, force, current_time)
            elif os.path.isfile(item_path) and item.startswith("converted_") and item.endswith(".zip"):
                self._maybe_delete_zip(item_path, force, current_time)

    def _maybe_delete_dir(self, dir_path: str, force: bool, current_time: float):
        try:
            ...
            self.logger.log(f"Error deleting folder {dir_path}: {e}", "error")

    def _maybe_delete_zip(self, zip_path: str, force: bool, current_time: float):
        try:
            ...
            self.logger.log(f"Error deleting ZIP file {zip_path}: {e}", "error")
