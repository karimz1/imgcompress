...
class BackendDiagnosticsService:
    ...
    def _document_lines(self) -> list[str]:
        return [
            ...
            "",
            "## Notes",
            "- This file contains backend startup output, app-level logs, and unhandled exceptions captured by imgcompress.",
            "- It only includes entries written after this backend process started.",
            "- Container runtime logs may still be useful if this file is empty or the process restarted.",
            "- For Docker, also attach `docker logs <imgcompress-container>` if available.",
            "- For local development, also attach the terminal output from `./runStartLocalBackend.sh`.",
            "",
